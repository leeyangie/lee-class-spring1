...
<body>
		...
		window.onload = () =>{
			...
			*/
			...
			function func(e) {
				...
				$('.modal-title').html($(e).l-title).test());
				$('#modal-writer').html($(e).find('.thfind('.thumbnaiumbnilwriter').text());
	...
	<div>
		<input type="text" id="j-btn">
		<button id="j-btn">버튼</button>
	</div>
	<script>
		$('#j-btn').click(() => {
			document.getElementById("j-input").value = '하하호호히히'
			alert(docunemt.getElementById("j-input").vlaue);			
		})
	</script>
					...
					<c:forEach items="${board }" var="img">
						<div class="col-md-4 col-sm-6 portfolio-item" onclick="detail(this);">
							...
							<a class="portfolio-link" data-toggle="modal" href="#MyModal">
								...
								<img class="img-thumbnail img-fluid thumbnamil-img" src="${img.changeName }" alt="게시글 이미지" >
							</a>
							<div class="portfolio-caption">
								<h4 class="thumbnamil-title">${img.boardTitle }</h4>
